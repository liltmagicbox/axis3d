...
def make_arr(value):
	try:
		return make_nd(DEFAULT_ARR_LENGTH, len(value) )
	except:
		return make_1d(DEFAULT_ARR_LENGTH)
#----
...
class UnitArray:
	def __init__(self, **kwargs):
		if not kwargs: raise KeyError("key:value is required to init UnitArray.")
		self.default = {}
		self.free_idxs = []
		self.active = None
		self.data = {}
	...
	def _setdefault(self, kwargs):
		...
		self._setdata()
	def _setdata(self):
		self.free_idxs = [i for i in range(DEFAULT_ARR_LENGTH)]
		self.active = make_b(DEFAULT_ARR_LENGTH)
		self.data = {}
		for key, value in self.default.items():
			self.data[key] = make_arr(value)
	#--------------
	def __len__(self):
	...
	def _extend(self, n):
		...
		if now_n+ext_n > MAX_ARR_LENGTH: raise OverflowError("array exceeds max len:"+str(MAX_ARR_LENGTH))
		#---
...
#how 7,7,7 to 3,1?
ua = UnitArray(hp=10,mp=5,pos=(7,7,7))
print(ua.default)
